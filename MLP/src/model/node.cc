...
auto Node::BuildOfWeights(const std::size_t neurons) -> void
{
    ...
    weights_ = Eigen::VectorXd::NullaryExpr(neurons, [&]() { return dis(gen) * scaling_factor; });
}

auto Node::UpdateWeights(double gradient, double learningRate, const Eigen::VectorXd &output)
    -> void
{
    double gamma = 0.9;
...
auto Node::GetWeights() const -> const Eigen::VectorXd &
{
    return weights_;
}

auto Node::Sigmoid(double value) -> double
{
    return (1 + std::exp(-value));
}

auto Node::Derivative() -> double
{
    return value_ * (1 - value_);
}

auto Node::SetWeights(Eigen::VectorXd weights) -> void
{
    weights_ = weights;
}
...
{
    os << node.weights_.size() << std::endl;
    os << node.weights_.transpose() << std::endl;
    return os;
}
